...
// 處理 TikTok Account（公司 TikTok 帳號授權）的 OAuth callback
// 對應 TikTok App 設定的 "TikTok account holder Redirect URL"
...
export default function handler(req, res) {
  ...
  return res.status(200).send(renderHTML({
    status: "success",
    title: "TikTok 帳號授權成功",
    message: "請複製下方的授權碼，貼到你的程式中換取 access_token（用來拉直播 / 影片 insights 數據）",
    code,
    state: state || "(none)",
    scopes: scopes || "(未指定)",
    type: "TikTok Account Holder (Display API)",
  }));
}

function renderHTML({ status, title, message, detail, code, state, scopes, type }) {
  ...
  <div class="card">
    ...
    ${code ? `
      ...
      <div class="field">
        ...
        <div class="value">${scopes || "(未指定)"}</div>
      </div>
