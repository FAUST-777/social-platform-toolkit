...
| 2026-05-21 | 發現 Sheet 重複寫入問題 | ad_reports.py 跑兩次導致花費 double，加去重保護修復 |
| 2026-05-21 | 新增 KOL成效排名 + 廣告組排名 獨立 tab | 從成效彙整拆出，方便同事快速查閱 |
| 2026-05-21 | 新增 QA Agent（qa_agent.py）| Claude Opus 4.7 自動核查 Sheet 格式 + API 數據一致性 |
| 2026-05-21 | TK大表 tab 最終確認 7 個 | safe cover / 廣告成效 / 成效彙整 / KOL排名 / 廣告組排名 / MKT建議 / QA報告 |
| 2026-05-21 | MKT + QA 全部清空重寫 | 清除舊報告，兩份均從第 1 列起，數據乾淨 |
| 2026-05-21 | QA 確認通過 | 231 筆，花費差異 0%，無嚴重問題，數據可信 |
...
- [x] ~~重複寫入問題~~ → 去重保護已上線（廣告ID + 日期唯一鍵）
- [x] ~~KOL成效排名 + 廣告組排名~~ → 獨立 tab，自動從廣告成效計算
- [x] ~~QA Agent~~ → qa_agent.py，每次更新後自動核查
- [ ] **廣告數據歸因修正**：24 個 KOL 活動花費歸戶異常（CTR 破萬%），需釐清 API 回傳層級問題
